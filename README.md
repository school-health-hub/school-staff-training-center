...
1. 저장소를 최신 main으로 업데이트
2. Apps Script `Code.gs`를 최신 버전으로 교체 후 재배포
3. 관리자 메뉴 > 시스템 업데이트에서 업데이트 확인
4. 업데이트 실행으로 없는 설정 key, 없는 헤더, `12_필수연수기준` 시트만 추가
5. 학교 설정에서 지역, 설립유형, 학교급 확인
...
- schemaVersion 기반 additive migration으로 기존 시트를 다시 복사하지 않고 업데이트 가능
...
- 필수연수 안내는 `12_필수연수기준`과 `01_교육목록`을 비교해 학교 전체 등록 현황만 보여줍니다.
- v1.1.0 기준 최신 schemaVersion은 `1.1.0`입니다.
