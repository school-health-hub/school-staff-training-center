...
- 관리자 화면은 `00_학교설정` 탭의 `adminCode` 값으로 최소 인증을 수행합니다. 브라우저에는 인증 성공 여부만 세션 동안 저장하며 관리자 코드 원문은 저장하지 않습니다. 이 기능은 학교 내부 운영용 간단 보호 장치이며, 외부 공개 서비스 수준의 완전한 접근 제어를 대체하지 않습니다.
